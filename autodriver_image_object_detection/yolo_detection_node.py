...
from autodriver_image_object_detection.utils.common import pack_nav2_obstacle_msg, pack_derived_object_msg, make_deleteall_marker_array
...
class ImageObstacleDetectionNode(BaseDetector):
    ...
    def _param_defaults(self) -> dict:
        """Overrides of the shared defaults, kept at this node's historical values."""
        pkg = get_package_share_directory('autodriver_image_object_detection')
        return {
            'model_path': 'yolo11x-seg.pt',
            'tracker_2d.path': os.path.join(pkg, 'config', 'tracker_custom.yaml'),
            'classes': ['person', 'car', 'chair'],
        }

    def _setup_device(self) -> None:
        """Pick the torch device, then the Open3D device for the pointcloud path."""
        super()._setup_device()
        if self.project_to_3d and self.use_pointcloud:
            # todo: add separate flag for Open3D
            self.o3d_device = o3d.core.Device('CPU:0')
            if self.use_gpu and o3d.core.cuda.is_available():
                self.o3d_device = o3d.core.Device('CUDA:0')

    def __init__(self):
        super().__init__("image_obstacle_detection_node")

        # Shared parameters (model, inference, tracking, QoS, debug image, ...)
        ...
        # Read the shared parameters into self.*
        self._read_common_params()

        # Read node-specific parameters
        ...
        # Device (also picks the Open3D device) and model
        self._setup_device()
        self.load_model()
    ...
    def parse_results(self, results, header):
        if results is not None:
            detections_msg, mask_img = self.create_detections_and_project(results, header)
    ...
    def create_detections_and_project(self, results, header):
        """Pack 2D detections and, when enabled, project each one into 3D.

        Deliberately not named create_detections_array: the base class method of
        that name takes a single Ultralytics result and returns
        (Detection2DArray, image, mask), whereas this takes the whole results list
        and also publishes the 3D arrays, markers and metric object arrays.
        """
        ...
        for result in results:
            # The 2D half — annotated image, segmentation mask composite, track
            # history polylines and the Detection2DArray itself — is the shared
            # BaseDetector implementation. Only the 3D projection below is
            # specific to this node.
            detections_2d, self.detection_image, mask_img = super().create_detections_array(
                result, header)
            detections_msg.detections.extend(detections_2d.detections)
            ...
            if self.project_to_3d:
                ...
                # masks.xy per detection drives the depth/pointcloud ROI; without a
                # segmentation model fall back to the 2D box (mask=None).
                masks = result.masks
                if masks is None:
    ...
    def parameter_change_callback(self, params):
        ...
        """
        # Common params (model/inference/tracking/debug-image) are handled by the
        # base class; everything below is specific to this node.
        result = super().parameter_change_callback(params)

        # Iterate over each parameter in this node
        for param in params:
            if param.name in self.COMMON_RECONFIGURABLE_PARAMS:
                continue
    ...
    def destroy_node(self):
        # delete the open3d pointcloud object cleanly before the shared teardown
        # (windows, model, CUDA cache, tracker temp file, rclpy node)
        if getattr(self, 'project_to_3d', False) and getattr(self, 'use_pointcloud', False):
